...
/// on subsystem `com.aerialrotate`, category `updater`) and the CLI:
/// `log stream --predicate 'subsystem == "com.aerialrotate"' --info`.
private let updateLogger = Logger(subsystem: "com.aerialrotate", category: "updater")
...
actor Updater {
    ...
    private static let logQueue = DispatchQueue(label: "com.aerialrotate.updatelog")
