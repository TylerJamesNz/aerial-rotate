...
/// (filter on subsystem `com.aerialrotate`, category `thumbnails`) and
/// the CLI: `log stream --predicate 'subsystem == "com.aerialrotate"' --info --debug`.
private let thumbnailLogger = Logger(subsystem: "com.aerialrotate", category: "thumbnails")
...
enum ThumbnailCache {
    ...
    private static let logQueue = DispatchQueue(label: "com.aerialrotate.thumblog")
